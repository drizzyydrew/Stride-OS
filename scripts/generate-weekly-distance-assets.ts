...
import {
  renderWeeklyDistanceBadgeSvg,
  type WeeklyDistanceBadgeState,
} from '../src/achievements/weeklyDistance/weeklyDistanceArtwork';
...
function renderPng(sourceSvgPath: string, targetPngPath: string): void {
  ...
  if (result.error) {
    throw new Error(`sips is required to render Weekly Distance PNG assets: ${result.error.message}`);
  }
  if (result.status !== 0) {
...
for (const definition of WEEKLY_DISTANCE_DEFINITIONS) {
  for (const variant of VARIANTS) {
    writeAsset(
      ...
      renderWeeklyDistanceBadgeSvg(definition.thresholdKm, variant.state),
    );
    svgCount += 1;
  }
...
console.log(`Generated ${svgCount} Weekly Distance SVG assets and ${pngCount} PNG assets.`);
