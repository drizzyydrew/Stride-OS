...
const ASSET_DIR = path.resolve(process.cwd(), 'assets/achievements/lifetime-distance-cycling');
const EXPECTED_VIEWBOX = '0 0 100 100';
const EXPECTED_PNG_SIZE = 1024;
const FORBIDDEN_INNER_DIAMOND_FRAGMENT = 'M50 15.2 Q51.7 15.2 53.1 16.6';
...
function normalizeOpaqueSvg(svg: string): string {
  return svg.replace(/lifetime-cycle-[^"]+-opaque-/g, match => match.replace('-opaque-', '-unlocked-'));
}
...
const expectedNames = new Set(
  LIFETIME_DISTANCE_CYCLING_DEFINITIONS.flatMap(definition => [
    `lifetime-cycle-${definition.slug}-unlocked.svg`,
    `lifetime-cycle-${definition.slug}-unlocked.png`,
    `lifetime-cycle-${definition.slug}-locked.svg`,
    `lifetime-cycle-${definition.slug}-locked.png`,
    `lifetime-cycle-${definition.slug}-transparent.svg`,
    `lifetime-cycle-${definition.slug}-transparent.png`,
    `lifetime-cycle-${definition.slug}-opaque.svg`,
    `lifetime-cycle-${definition.slug}-opaque.png`,
  ]),
);
const actualNames = new Set(readdirSync(ASSET_DIR));
for (const name of expectedNames) assert(actualNames.has(name), `${name} missing from lifetime-cycling asset directory`);
for (const name of actualNames) assert(expectedNames.has(name), `${name} is not a deterministic lifetime-cycling asset filename`);

console.log(`Validated ${checked} Lifetime Distance - Cycling assets across ${LIFETIME_DISTANCE_CYCLING_DEFINITIONS.length} milestones.`);
