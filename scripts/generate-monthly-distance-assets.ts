...
import {
  renderMonthlyDistanceBadgeSvg,
  type MonthlyDistanceBadgeState,
} from '../src/achievements/monthlyDistance/monthlyDistanceArtwork';
...
function renderPng(sourceSvgPath: string, targetPngPath: string): void {
  ...
  if (result.error) {
    throw new Error(`sips is required to render Monthly Distance PNG assets: ${result.error.message}`);
  }
  if (result.status !== 0) {
...
for (const definition of MONTHLY_DISTANCE_DEFINITIONS) {
  for (const variant of VARIANTS) {
    writeAsset(
      ...
      renderMonthlyDistanceBadgeSvg(definition.thresholdKm, variant.state),
    );
    svgCount += 1;
  }
...
console.log(`Generated ${svgCount} Monthly Distance SVG assets and ${pngCount} PNG assets.`);
