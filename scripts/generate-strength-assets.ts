...
import { renderStrengthAchievementBadgeSvg } from '../src/achievements/strength/strengthArtwork';
...
function renderPng(sourceSvgPath: string, targetPngPath: string): void {
  ...
  if (result.error) {
    throw new Error(`sips is required to render Strength PNG assets: ${result.error.message}`);
  }
  if (result.status !== 0) {
...
for (const definition of STRENGTH_ACHIEVEMENT_DEFINITIONS) {
  for (const variant of VARIANTS) {
    writeAsset(
      ...
      renderStrengthAchievementBadgeSvg(definition.id, variant.state),
    );
    svgCount += 1;
  }
...
const manifest = STRENGTH_ACHIEVEMENT_DEFINITIONS.map(definition => ({
  id: definition.id,
  slug: definition.slug,
  title: definition.title,
  glyph: definition.glyph,
  includeInGlobalRegistry: definition.includeInGlobalRegistry,
...
writeAsset('assets/achievements/strength/manifest.json', `${JSON.stringify(manifest, null, 2)}\n`);

console.log(`Generated ${svgCount} Strength SVG assets and ${pngCount} PNG assets.`);
